...
if [ -f composer.lock ]; then
    ...
    LOCK_HASH_REMOTE=$(ssh -i ~/.ssh/id_rsa -o StrictHostKeyChecking=no ${{ secrets.SSH_USERNAME }}@${{ secrets.SSH_HOST }} "md5sum /var/www/html/laravelcicd/composer.lock | awk '{ print $1 }'")
